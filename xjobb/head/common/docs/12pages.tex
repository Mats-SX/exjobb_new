...
\begin{figure}
\centering
 \begin {tikzpicture}[-latex, auto, on grid, semithick, scale=.7,inner sep=0,
b/.style ={ circle, draw, text=white, minimum width =5 mm, fill=black},
g/.style ={ circle, draw, text=blue, minimum width =5 mm, fill=gray},
w/.style ={ circle, draw, text=black, minimum width =5 mm, fill=white}]
\node[g] (1) at (-2,-2) {$1$};
\node[b] (2) at (-2,3) {$2$}
  edge [-] (1);
\node[w] (3) at (3,3) {$3$}
...
\node[g] (24) at (9,0) {$24$}
  ...
  edge [-] (23);
\end{tikzpicture}
 \caption{Akbari's graph, coloured in its unique 3-colouring with white, gray and black as colours. Figure copied from figure~2 in \cite{hillar_windfeldt}. This graph was used as a counter-example to disprove a conjecture due to Shaoji Xu (see \cite{akbari}), as it has $(\chi(G) - 1)n - \binom{\chi(G)}{2} = 45$ edges but is triangle-free.}
 \label{akbaris}
\end{figure}
\end{center}
